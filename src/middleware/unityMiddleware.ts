...
export interface UnityQueryValidationOptions {
    allowSearch?: boolean;
    allowedSortFields?: string[];
}

export const validateUnityQuery = (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;

    if (!id || isNaN(Number(id))) {
        return res.status(400).json({ message: UNITY_ERROR.ID });
    }

    next();
};

export function validateUnitiesQuery(options: UnityQueryValidationOptions = {}) {
    ...
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        if (!allowSearch && search !== undefined) {
            return res.status(400).json({ message: UNITY_ERROR.SEARCH });
        }
        ...
        if (sortBy && !allowedSortFields.includes(sortBy.toString())) {
            return res.status(400).json({ message: UNITY_ERROR.SORT_BY });
        }

        if (sortOrder && sortOrder !== "asc" && sortOrder !== "desc") {
            return res.status(400).json({ message: UNITY_ERROR.SORT });
        }
